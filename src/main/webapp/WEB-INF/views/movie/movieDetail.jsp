...
function showMyReview() {
	...
	reviewService.getListMyReview(movie_pk, userid, function(list) {
		...
		if(list.length) { // 작성한 리뷰가 있을때
			...
			str += "	  <td class='rv_title pt-3'>" + list[0].rv_title.replace(/(<([^>]+)>)/ig, "") + "</td>";
			...
			str = "<tr class='text-center' style='background: #f9f3c7;'>" +
			"<td colspan='5'>" +
			"<a href='/user/login' class='text-muted btn-review-write'>작성하신 리뷰가 없습니다. 리뷰를 작성하시려면 이 곳을 클릭해주세요.</a>" +
			"</td>" +
			"</tr>";
		}
		$(".myreviewTB").html(str);
...
function showList(page) {
	...
	function(reviewCnt, score, list) {
		...
		for (var i = 0; i < list.length; i++) {
			...
			str += "	  <td class='rv_title'>" + list[i].rv_title.replace(/(<([^>]+)>)/ig, "") + "</td>";
			str += "	  <td class='text-center score text-warning' style='font-size:20px;'>";
	...
	$(".reviewTB, .myreviewTB").on("click", "tr[data-review_pk]", function(e) {
		...
		reviewService.get(review_pk, function(myreview) {
			...
			if(modalInputUserid == userid) {
				modal.find("input, textarea").removeAttr("readonly");
				modal.find("select").removeAttr("disabled");
				
			} else {
				modal.find("#modBtn").hide();
				modal.find("#rmBtn").hide();
				modal.find("input, textarea").attr("readonly", "readonly");
					...
					<div class="col mx-auto p-0">
						...
						<div class="container row">
							<div class="col p-0 mt-3 mb-3">
								...
								<div class="container">
									<div class= "col-sm-9 container float-left row">
										<div class="mt-1">
									...
									<div class="col-sm-3 float-right row pr-0">
										<div>
																...
																<div class="card-body modal-body3">
																	<form class="form-signin" role="form"
																		...
																		<div class="container modal-body4">
																			<!-- 제목, 내용 구간 -->
																			<div>
																				<div class="form-label-group mb-3" id="test">
																			...
																			<div class="mb-4">
																				...
																				<select	class="custom-select form-control" id="sel2"
																					name="score">
